...
async fn open_manual_tasks_appear_in_context() {
    ...
    let pm = users::create(
        &pool,
        "Ctx PM",
        &format!("ctxpm-{tag}@nolinear.local"),
        "h",
    ...
    let open = manual_tasks::create(&pool, emp.id, pm.id, "Open task", "do this")
        .await
        .unwrap();
    let done = manual_tasks::create(&pool, emp.id, pm.id, "Done task", "already finished")
        .await
        .unwrap();
    ...
    let ctx = analysis_service::build_context(&pool, &linear, emp.id)
        .await
        .unwrap();

    let ids: Vec<&str> = ctx.iter().map(|t| t.id.as_str()).collect();
    assert!(
        ids.contains(&format!("task:{}", open.id).as_str()),
        "open task should be in context"
    );
    assert!(
        !ids.contains(&format!("task:{}", done.id).as_str()),
        "done task must be excluded"
    );

    let entry = ctx
        .iter()
        .find(|t| t.id == format!("task:{}", open.id))
        .unwrap();
