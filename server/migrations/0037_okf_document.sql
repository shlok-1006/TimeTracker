-- OKF: the company rulebook, a single HR-editable document served in admin-web.
-- One canonical row (id = 1); HR edits it via GET/PUT /admin/okf. Seeded here from
-- the versioned OKF.md so the running system starts with the captured ruleset.
CREATE TABLE IF NOT EXISTS okf_document (
    id         SMALLINT PRIMARY KEY DEFAULT 1 CHECK (id = 1),
    content    TEXT NOT NULL,
    updated_by UUID REFERENCES users (id) ON DELETE SET NULL,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
INSERT INTO okf_document (id, content) VALUES (1, $okfdoc$# OKF — Company Rulebook (Single Source of Truth for Policy)
