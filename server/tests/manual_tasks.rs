...
async fn manual_task_crud_roundtrip() {
    ...
    // Mark done.
    assert!(manual_tasks::set_status(&pool, task.id, "done")
        .await
        .unwrap());
    assert_eq!(
        manual_tasks::get(&pool, task.id)
            .await
            .unwrap()
            .unwrap()
            .status,
    ...
    assert!(manual_tasks::list_for_user(&pool, emp.id)
        .await
        .unwrap()
        .is_empty());
