...
async fn list_for_day_filters_with_verdict_and_meeting_flag() {
    ...
    let job = sampler::create_daily_job(&pool, user.id, day_a)
        .await
        .unwrap();
    ...
    let a = screenshots::list_for_day(&pool, user.id, day_a)
        .await
        .unwrap();
    ...
    let b = screenshots::list_for_day(&pool, user.id, day_b)
        .await
        .unwrap();
    ...
    let empty =
        screenshots::list_for_day(&pool, user.id, NaiveDate::from_ymd_opt(2099, 3, 3).unwrap())
            .await
            .unwrap();
